...
void init_timeout() {
  ...
  //  primanje(&newsockfd,new_interrupt_list,cpu_usage1,new_task_list,new_device_list,network,tm1,memory_usage);
    ...
    //primanje2(&newsockfd,new_interrupt_list,cpu_usage1,new_task_list,new_device_list,network,memory_usage);
 ...
 /*   for(int u=0;u<4;u++){
        ...
        Cpu_usage *array_cpu;
         array_cpu = &cpu_usage1[u];
        printf("Number %" SCNi32 " usage %f\n",array_cpu->number,array_cpu->percentage);
