    ...
    if (transmit_bytes_old == 0) {
        ...
        *transmited_struct = 0;
        ...
        printf("Received_struct  %" SCNu64 " Transimeted_struct %" SCNu64 " received %" SCNu64 " transimted %" SCNu64 " "
               ...
               received, transmitted, received_bytes_old, transmit_bytes_old
...
//    int i=0;
    ...
    while ((pDirent = readdir(pDir)) != NULL) {
        ...
        if (!strcmp(pDirent->d_name, ".") || !strcmp(pDirent->d_name, "..") || !strcmp(pDirent->d_name, "lo")) {
            ...
            //  printf("buffer2 [%s]\n", buffer2);
            printf(">>>5\n");
            sscanf(buffer2, " %" SCNu64 " %lu  %lu %lu %lu %lu %lu %lu %" SCNu64 " %lu %lu %lu %lu %lu %lu %lu",
                   &received_bytes,
                   &received_packets,
    ...
    printf(" Received  [%" SCNu64 "] Transmitted [ %" SCNu64 "] \n", network1->received_bytes,
           network1->transmited_bytes);
    printf(">>>7\n");
    broj = 0;
    closedir(pDir);
    printf("exit %s in %s:%d \n", __FUNCTION__, __FILE__, __LINE__);
